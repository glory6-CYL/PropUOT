...
class InHospitalMortalityReader(Reader):
    def __init__(self, dataset_dir, listfile=None, period_length=48.0):
        ...
        """
        Reader.__init__(self, dataset_dir, listfile)
        self._data = [line.strip().split(',') for line in self._data]
        # Listfile columns: stay, period_length, stay_id, y_true.
        self._data = [(parts[0].strip(), int(parts[3])) for parts in self._data if len(parts) >= 4]
