...
def read_patients_table(path):
    pats = pd.read_csv(_csv_path(path))
...
def read_admissions_table(path):
    ...
    admits = pd.read_csv(_csv_path(path))
...
def read_icustays_table(path):
    stays = pd.read_csv(_csv_path(path))
...
def read_icd_diagnoses_table(path):
    ...
    codes = pd.read_csv(_csv_path(f'{path}/d_icd_diagnoses.csv'))
    # dataframe_from_csv(os.path.join(mimic3_path, 'D_ICD_DIAGNOSES.csv'))
    codes = codes[['icd_code', 'long_title']]
    diagnoses = pd.read_csv(_csv_path(f'{path}/diagnoses_icd.csv'))
...
def read_events_table_by_row(mimic3_path, table):
    ...
    with _open_csv(os.path.join(mimic3_path, csv_files[table.lower()])) as handle:
