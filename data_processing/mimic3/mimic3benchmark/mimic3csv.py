...
def _csv_path(path):
    """Return either an unpacked CSV or the original PhysioNet CSV.GZ."""
    if os.path.isfile(path):
        return path
    compressed = path + '.gz'
    if os.path.isfile(compressed):
        return compressed
    raise FileNotFoundError('Missing MIMIC table: {} (or .gz)'.format(path))


def _open_csv(path):
    resolved = _csv_path(path)
    return gzip.open(resolved, 'rt', newline='') if resolved.endswith('.gz') else open(resolved, 'r', newline='')


def read_patients_table(mimic3_path):
    pats = dataframe_from_csv(_csv_path(os.path.join(mimic3_path, 'PATIENTS.csv')))
...
def read_admissions_table(mimic3_path):
    admits = dataframe_from_csv(_csv_path(os.path.join(mimic3_path, 'ADMISSIONS.csv')))
...
def read_icustays_table(mimic3_path):
    stays = dataframe_from_csv(_csv_path(os.path.join(mimic3_path, 'ICUSTAYS.csv')))
...
def read_events_table_by_row(mimic3_path, table):
    ...
    with _open_csv(os.path.join(mimic3_path, table.upper() + '.csv')) as handle:
