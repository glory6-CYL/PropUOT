"""MIMIC-CXR report section parser.

Adapted from the MIT-LCP MIMIC-CXR code repository (MIT license); see
data_processing/third_party_licenses/mit_lcp_mimic_cxr.txt.
"""

import re


TARGETS = ("impression", "findings", "last_paragraph", "comparison")
...
def extract_sections(text, study=None):
    """Return the four report fields consumed by PropUOT."""
    sections, names, _ = section_text(text)
    custom_names, custom_indices = custom_mimic_cxr_rules()
    result = {name: "" for name in TARGETS}

    if study in custom_indices:
        start, end = custom_indices[study]
        result["impression"] = text[start:end].strip()
        return result

    if study in custom_names and custom_names[study] in names:
        index = len(names) - names[::-1].index(custom_names[study]) - 1
        result["impression"] = sections[index].strip()

    for target in TARGETS:
        if target in names:
            index = len(names) - names[::-1].index(target) - 1
            result[target] = sections[index].strip()
    return result
